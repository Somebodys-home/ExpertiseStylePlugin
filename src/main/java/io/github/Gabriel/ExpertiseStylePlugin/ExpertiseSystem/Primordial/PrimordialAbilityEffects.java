...
public class PrimordialAbilityEffects {
    ...
    public static void pumpkinBomb(Player user, int hotbarSlot) {
        ...
        HashMap<DamageType, Double> weapon = DamageConverter.multiplyDamageMap(DamageConverter.convertPlayerStats2Damage(profileManager.getPlayerProfile(user.getUniqueId()).getStats()), .5);
            weapon.remove("earthdamage");
            weapon.remove("firedamage");
