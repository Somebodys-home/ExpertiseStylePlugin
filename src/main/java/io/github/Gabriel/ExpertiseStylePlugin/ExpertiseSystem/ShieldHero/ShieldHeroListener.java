...
public class ShieldHeroListener implements Listener {
    ...
    private ShieldHeroAbilityEffects shieldHeroAbilityEffects;
    ...
    public void onUseAbility(UseAbilityEvent event) {
        ...
        shieldHeroAbilityEffects = new ShieldHeroAbilityEffects(expertiseStylePlugin, event.getPlayer(), event.getHotbarSlot());

        if (Arrays.asList(selectedAbilities).contains(abilityName)) {
            if (ExpertiseManager.isHoldingWeaponForAbility(event)) {
                if (abilityName.equals("§3§lSecond Wind")) shieldHeroAbilityEffects.secondWind();
            } else {
